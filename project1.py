...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    loss = np.maximum(1 - z * labels, np.zeros(len(labels)))
    # Over all of the datas
    return np.mean(loss)
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    (nsamples, nfeatures) = feature_matrix.shape  # Return dimension of matrix
...
def pegasos_single_step_update(
    ...
    if label * (np.dot(current_theta, feature_vector) + current_theta_0) <= 1:
        return (
            (1 - L * eta) * current_theta + eta * label * feature_vector,
